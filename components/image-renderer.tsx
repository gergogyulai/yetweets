...
export default function MediaRenderer({ media }: { media: Media[] }) {
  if (media[0].expanded_url?.includes('video')) return <div className="text-muted-foreground text-sm mt-2">Video media is not supported at all.</div>
      ...
      {media.map((item, index) => {
        ...
        return (
          <Link
            ...
            key={item.id_str}
          >
            <div className={`${getImageClass(index, media.length)} relative cursor-pointer overflow-hidden`} style={{ paddingBottom: '100%' }}>
              <Image
                src={imageUrl}
                alt="Tweet Media"
                fill
